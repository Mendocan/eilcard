...
- [x] En az bir `write:` veya `act:` scope ile ayrı consent (E3-C)
- [x] Idempotent POST with `Idempotency-Key` replay test
- [x] Token `eil_card_id` claim ile domain'e bağlı
- [x] Revoke sonrası 401 (`POST /oauth/revoke` + E2E)
- [x] Audit log (platform tarafı — `GET /v1/audit/recent` pilot)

...
| `/v1/posts` | POST | Bearer `write:post` + `Idempotency-Key` |
| `/v1/act/comment` | POST | Bearer `act:comment` + `Idempotency-Key` |
| `/v1/audit/recent` | GET | Bearer `read:profile` (pilot audit tail) |
| `/health` | GET | Liveness |
...
```json
{
  "capabilities": {
    ...
    "actions": [
      { "id": "create_post", "method": "POST", "path": "/v1/posts", "scopes": ["write:post"] },
      { "id": "add_comment", "method": "POST", "path": "/v1/act/comment", "scopes": ["act:comment"] }
    ]
  }
}
```
...
- Kalıcı token store, refresh token
- Production hardening (rate limit, mTLS, persistent audit export)

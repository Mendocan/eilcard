...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const doc = legalMessages[locale].terms;
  return {
    title: doc.metaTitle,
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-4 py-16 sm:px-6 sm:py-20">
        <LegalDocumentBody doc={legal.terms} />
        <div className="mt-10 flex flex-wrap gap-4 text-sm">
          <Link href="/legal/privacy" className="text-[var(--color-accent)] hover:opacity-80">
            {m.footer.privacy}
          </Link>
          <Link href="/legal/refund" className="text-[var(--color-accent)] hover:opacity-80">
            {m.footer.refunds}
          </Link>
